...
//总结来看，用<int>更好，无穷大可以用INT_MAX来表示
//全局动态数组
...
bool ALGraph<VertexType>::DeleteVertex(const VertexType& vex)
{
    ...
    for (int i = 0; i < numVertices; i++)
    {
            ...
            if (vertices[i].RemoveEdge(v)||vertices[v].RemoveEdge(i))
            {
                --numEdges;
            }
...
void ALGraph<VertexType>::GetDetail()
{
	...
	for(int i=0;i<numVertices;i++)
	{
		...
		else
		{
			for(int j=0;j<numVertices;j++)
			{
				if(array[i][j]!="#"&&array[i][j]!="0") numCHU++;
				if(array[j][i]!="#"&&array[i][j]!="0") numRU++;
			}
...
//--------------------Kruskal算法---------start--------
//哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇哇！泪，涌了下来！
struct NotusedW
{
...
void Kruskal(ALGraph<VertexType>& g)
{
    ...
    NotusedW notusedw[g.GetNumEdges()*2];
    ...
    //从权值最小的边开始，如果不在同一个连通分图中，则合并为同一连通分图
    sort(notusedw,notusedw+k,cmmp);
    for(int i=0;i<k;i++)
    {
        if(m[notusedw[i].front]!=m[notusedw[i].back])
        {
            ...
            VertexType v1,v2;
            g.GetVertex(notusedw[i].front,v1);
            g.GetVertex(notusedw[i].back,v2);
            cout<<v1<<"->"<<v2<<",权值:"<<notusedw[i].wei<<endl;
            sum+=notusedw[i].wei;
        }
    }
...
// 用Dijkstra算法求有向图g的顶点vex到其余顶点的最短路径，返回TableEntry类型数组(因为这个算法是把全部的两点间最短路径求出来，把这个数组给外部用)
template <class VertexType>
...
template <class VertexType>
int* ShortestPath_F(ALGraph<VertexType>& g,VertexType vex)
{
    int **dist;
    string **mid;
    VertexType v1,v2;
    int start=g.LocateVertex(vex);
    dist=new int*[g.GetNumVertices()];
    for(int i=0;i<g.GetNumVertices();i++)
    {
        dist[i]=new int[g.GetNumVertices()];
    }
    mid=new string*[g.GetNumVertices()];
    for(int i=0;i<g.GetNumVertices();i++)
    {
        mid[i]=new string[g.GetNumVertices()];
    }
    for(int i=0;i<g.GetNumVertices();i++)
      for(int j=0;j<g.GetNumVertices();j++)
      {
          g.GetVertex(i,v1);g.GetVertex(j,v2);
          mid[i][j].push_back(v1);mid[i][j].push_back(v2);
          if(array[i][j]!="#")
          {
              dist[i][j]=std::stoi(array[i][j]);
          }
          else
          {
              dist[i][j]=INT_MAX;
      ...
      //每次通过顶点序号为i,判断任意两点是否通过i点的传递是否会更短
    for(int i=0;i<g.GetNumVertices();i++)
    {
        for(int j=0;j<g.GetNumVertices();j++)
        {
            ...
            {
                if(dist[j][i]+dist[i][k]<dist[j][k]&&dist[j][i]!=INT_MAX&&dist[i][k]!=INT_MAX)
                {
                    dist[j][k]=dist[j][i]+dist[i][k];
                    mid[j][k]=mid[j][i]+mid[i][k];
                }
            }
        } 
    }
    for(int i=0;i<g.GetNumVertices();i++)
    {
        if(dist[start][i]!=INT_MAX)
        {
            char c;
            g.GetVertex(i,c);
            cout<<vex<<"->"<<c<<" 的最短距离："<<dist[start][i]<<"   ";
            cout<<"最短路径为：";
            for(int j=0;j<mid[start][i].length()-1;j++)
            {
                if(mid[start][i][j]!=mid[start][i][j+1])
                {
                    cout<<mid[start][i][j]<<"->";
                }
            }
            cout<<mid[start][i][mid[start][i].length()-1];
        }
        else
        {
            char c;
...
{
    char vex;
    int *d=new int[a.GetNumVertices()];
    cout<<"Floyd算法求得顶点之间得最短路径"<<endl;
    cout<<"请输入一个顶点（值）：";
    cin>>vex;
    d=ShortestPath_F(a,vex);
    cout<<"\npress any key to return to the menu"<<endl;
    getchar();getchar();
}
